...
解析失败时,把错误用**中文**回喂模型重试。本库不绑定传输——模型怎么调由你在 `call` 里决定:

```ts
import { coerceWithRetry } from "coerce-llm";

const r = await coerceWithRetry({
  schema,
  maxRetries: 2, // 最多重试 2 次(共调用 3 次)
  call: async (hint, attempt) => {
    // hint:首次为 undefined;重试时是中文纠错提示,拼进你的 prompt 即可
    const messages = [{ role: "user", content: basePrompt + (hint ? `\n\n${hint}` : "") }];
    return await myModel(messages); // 返回原始字符串
  },
});

if (r.ok) console.log(r.data, `共 ${r.attempts} 次`);
```

hint 会指出具体问题,例如 schema 不符时:`字段 severity: Invalid enum value...`,让模型有的放矢地改。

## 流式:边流边解析

模型流式返回时,普通 `JSON.parse` 遇到半截会炸。`parsePartial` 能从残缺 JSON 里解析出当前的局部对象,`coerceStream` 则消费 chunk 流、边流边回调、流末做正式校验:

```ts
import { coerceStream, parsePartial } from "coerce-llm";

// 纯函数:半截 JSON → 当前局部对象
parsePartial('{"title":"登录逻辑可能空指'); // → { title: "登录逻辑可能空指" }

// 消费文本块流:onPartial 实时更新 UI,返回值是最终校验结果
const result = await coerceStream(chunkStream, schema, (partial) => {
  render(partial); // 标题先出现、字段逐步长出,像打字机
});
...
- **不调用模型**、不管网络/鉴权——那是 SDK 或你自己的事(`coerceWithRetry` 也只在你给的 `call` 里调,库本身不发请求)。
...
- [x] 带模型重试的 `coerceWithRetry`:把 zod 错误用中文回喂模型纠错
- [x] 流式部分 JSON 解析(`parsePartial` / `coerceStream`)
- [ ] 值级轻度强转(数字字符串 → 数字等)
- [ ] 更多真实脏样本进测试集
